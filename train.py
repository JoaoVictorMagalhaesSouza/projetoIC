...
import os, sys
dir_import = os.path.dirname(os.path.realpath(__file__))
...
class_fe = FeatureEngineeringFactory(df)
...
# %% Building LSTM
model = LSTM(X_train,x_test,y_train,y_test)
model.fit()
model.predict()
model.evaluate()
model.plotCurve()
...
# %% Building CatBoost
model = CatBoost(X_train,x_test,y_train,y_test)
model.fit()
model.predict()
model.evaluate()
model.plotCurve()
